...
use_saved_model_general = configuration["train"]["use saved"]
...
for selected_model in model_options:
    if verbose:
        print(f"Train: {selected_model}")
    
    use_saved_model = use_saved_model_general
    ...
    train_loader = utils.create_loader(utils.load_images(os.path.join(DATA_PATH, data_division[0]), get_mask, verbose = verbose), batch_size=batch_size)
